...
1. 最有代表性的是“从平台收到设备信号，到可操作 Alert 在页面/API 中可见的端到端延迟”，例如按严重度定义某百分位在约定秒数内完成。它同时覆盖接收器、解析、数据库、规则和查询，而不是只测 Web 200。通知送达可以作为第二条 SLO，因为外部邮件故障不应掩盖 Alert 已经生成的事实；具体阈值必须由业务值班要求决定，仓库没有现成生产数字。

2. 当前系统没有持久消息缓冲，因此在继续接收 UDP 的同时无法严格保证数据库维护期间零丢失。可行窗口应先让发送方切换到备用接收路径或启用持久缓冲，再停止新写入、等待在途事务完成、做可恢复备份/故障切换，完成迁移后先验证 readiness 和 Flyway，再恢复流量并核对事件序号。若上游不能缓冲，就必须把可能丢失明确计入维护风险，而不能口头承诺“零丢失”。

3. 外部系统应采集 HTTPS 与协议端口的合成探测、端到端事件到 Alert 延迟、LB/VPN/DNS 可达性、集中日志到达率，以及数据库备份可恢复演练结果。安全侧还需要独立漏洞/镜像扫描和身份审计留存。应用自身报告的 health、metrics 和“备份成功”日志只能作为内部信号，不能单独证明用户路径、网络边界或恢复能力。
